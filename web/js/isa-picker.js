...
function extractEntriesFromData(data, isaType) {
    ...
    if (data.entries && Array.isArray(data.entries)) {
        data.entries.forEach(entry => {
            ...
            const isaType = entry['ISA.type'] || entry.type || '';
...
function addInfoFromIsa(type, isaItem) {
    ...
    const infoId = `${type}_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
    const label = type === 'cde' ? 'CDE' : (type === 'alarmes' ? 'Alarme' : 'TCD');

    const infoHtml = `
        <div class="info-item" id="${infoId}">
            <input type="text" value="${escapeHtml(isaItem.libelle8 || isaItem.id)}"
    ...
    // Ajouter au modèle
    if (typeof currentTest !== 'undefined' && currentTest[type]) {
